...
type game =
  { g_map : Data.map; g_params : Protocol.Initialisation.params;
    g_players : (string, (string * char)) Hashtbl.t;
    g_start : (Unix.tm * int); g_name : string
  }
...
module Server =
  struct
    ...
    type state =
      ...
      { mutable next_id : int; (* Set of available characters on the map *)
        available : (char, unit) Hashtbl.t; (* The world map *)
        map : Data.map; (* Game parameters *) params : params;
        (* All registered players *)
        players : (string, (string * char)) Hashtbl.t;
        mutable starting : (Unix.tm * int) option;
        (* Called on game update *) updated : unit Lwt_condition.t;
        meta : Meta.Client.Connection.t; game : Protocol.Meta.game
      }
    
    let mk_state map params meta game =
      let available = Hashtbl.create 17
      in
    ...
    let nb_players { players = players; _ } = Hashtbl.length players
      
    let max_players { map = map; _ } = Data.map_nb_players map
      
    (* Treat message as authentified user [cid] in state [state] *)
    let treat_message cid state =
      function
      | HELLO _ ->
          ((Some cid),
           [ REJECTED "There can be only one user per connection." ])
      
    (* Treat end of connection as authentified user [cid] in state [state] *)
    let treat_end cid state () =
      (try
         let (_, chr) = Hashtbl.find state.players cid
         in
       ...
       | Not_found ->
           Lwt.bind
             (Lwt_log.fatal
                ("Client ID not found in state. Continuing could lead to " ^
                   ("severe inconsistencies, aborting everything. This is a programmer "
                      ^ "failure.")))
             (fun _ -> assert false))
        >>=
        (fun () -> (* Broadcast quitting *)
           (Lwt_condition.broadcast state.updated (); return ()))
      
    exception Found of char
      
    let treat_anonymous_message state =
      function
      | HELLO (pseudo, versions) -> (* Protocol check *)
          if not (List.mem 1 versions)
          then
            (None,
             [ REJECTED
                 ("Only version 1 of the protocol is supported by this" ^
                    " server.") ])
          else (* Are there still places available ? *)
            if (nb_players state) >= (max_players state)
            then
              (None,
               [ REJECTED "There is no room for an additional player." ])
            else (* Is game starting ? *)
              if state.starting <> None
              then (None, [ REJECTED "Game is already starting. Sorry!" ])
              else (* Check that the pseudo is available *)
                (let duplicated =
                   Hashtbl.fold (fun _ (o, _) v -> v || (o = pseudo))
                     state.players false
                 in
                   if duplicated
                   then
                     (None,
                      [ REJECTED
                          "There is already a player with this pseudo." ])
                   else (* Compute ID *)
                     (let c =
                        try
                          (Hashtbl.iter (fun k () -> raise (Found k))
                             state.available;
                           (* Impossible: [nb_players state < max_players state] here *)
                           Lwt.ignore_result
                             (Lwt_log.fatal
                                ("Unavailable ID while there shall still be room "
                                   ^
                      ...
                      in
                        (Hashtbl.remove state.available c;
                         let s = string_of_int state.next_id
                         in
             ...
             * investigate (TODO). *)
                            ...
                            in
                              ...
                              (Hashtbl.add state.players s (pseudo, c);
                               ...
                               (* Send messages *)
                               ((Some s),
                                ((OK (s, state.map, state.params)) :: joins)))))))
    ...
    let treat state stream =
      ...
      and server_stream =
        Lwt_stream.flatten
          (Lwt_stream.from
             (fun () ->
                Lwt.bind (Lwt_condition.wait state.updated)
                  (fun _ ->
                     let startings =
                       match state.starting with
                       | None -> []
                       | Some (x, y) -> [ `Start (x, y) ] in
                     let joins =
                       Hashtbl.fold
                         (fun id (pseudo, pos) l ->
                            try
                              if
                                (Hashtbl.find my_players id) <> (pseudo, pos)
                              then
                                (`Quit id) :: (`Join (pseudo, id, pos)) :: l
                              else l
                            with
                            | Not_found -> (`Join (pseudo, id, pos)) :: l)
                         state.players startings in
                     let quits =
                       Hashtbl.fold
                         (fun id (pseudo, pos) l ->
                            if not (Hashtbl.mem state.players id)
                            then (`Quit id) :: l
                            else l)
                         my_players joins
                     in
                       (Hashtbl.clear my_players;
                        Hashtbl.iter (fun k v -> Hashtbl.add my_players k v)
                          state.players;
                        return (Some quits))))) in
      let real_stream = merge ~quit: true [ client_stream; server_stream ] in
      let rec treat_input do_message do_end =
        (Lwt_stream.get real_stream) >>=
          (function
           | Some (`Client msg) ->
               let (cli, lst) = do_message state msg
               in
                 (List.iter (fun e -> push (Some e)) lst;
                  ...
                  (match cli with
                   ...
                   | None ->
                       treat_input treat_anonymous_message (fun _ -> return)))
           | Some (`Start (x, y)) ->
               (push (Some (START (x, y))); push None; return ())
           | Some (`Join (s, s', c)) ->
               (push (Some (JOIN (s, s', c))); treat_input do_message do_end)
           | Some (`Quit a) ->
               (push (Some (QUIT a)); treat_input do_message do_end)
           | None -> (push None; do_end state ()))
      in
        (Lwt.async
           (fun () -> treat_input treat_anonymous_message (fun _ -> return));
         out)
      
    let rec handle_server server state =
      let cur_players = nb_players state
      in
        Lwt.bind (Lwt_condition.wait state.updated)
          (fun _ ->
             (if (nb_players state) <> cur_players
              then
                Meta.Client.update state.meta
              ...
              else handle_server server state))
      
    let create addr meta =
      Unix.handle_unix_error
        (fun () -> (* Load the world, TODO ? *)
           (Lwt_stream.to_string (Lwt_io.chars_of_file "world")) >>=
             (fun s ->
                let map = Data.map_of_string s
                in
                  (Meta.Client.add meta ~addr ~name: "CATSERV"
                     ~nb_players: (Data.map_nb_players map))
                    >>=
                    (function
                     | None -> assert false
                     | (* TODO *) Some game ->
                         ...
                         let server =
                           Connection.establish_server addr
  ...
  struct
    ...
    type t =
      (Connection.t * Lwt_mutex.t * (event Lwt_stream.t) *
       ([
          | `Rejected of string
          | `Ok of (string * Data.map * Protocol.Initialisation.params) ]
          Lwt_stream.t))
    
    let connect addr =
      (Connection.open_connection addr) >>=
        (fun co ->
