...
and read_bdict s i hash k =
  get s i (fun s -> function
    | 'e' -> k s ( D hash, i + 1)
    | c ->
        read_bstring s i (fun s (key, i') ->
          match key with
          | S skey ->
            process s i' (fun s (v, i'') ->
              Hashtbl.add hash skey v;
...
and start s i k = function
    ...
    (* | '\n' -> get s (i + 1) (fun s -> start s (i + 1) k) *)
    (* | _ -> raise ( Format_error ) *)
    | _ -> get s (i + 1) (fun s -> start s (i + 1) k)
  
let empty_fun = fun s -> start s 0
    (fun (s, init) (x, i) ->
      let len = String.length s in
      Val (x, String.sub s (i - init) (len + init - i)))
...
let rec encode = function
  ...
  | D h ->
      "d" ^ Hashtbl.fold (fun k v s -> s ^ encode (S k) ^ encode v) h "" ^ "e"
...
module WrapBufferize(W : Wrapper) = struct
  ...
  let reset_buffer = Bufferize.reset_buffer
end
