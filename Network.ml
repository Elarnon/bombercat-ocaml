...
let dns_resolve addr =
  let __pa_lwt_0 = U.gethostbyname addr
  in
    Lwt.bind __pa_lwt_0
      (fun host ->
         let l = host.U.h_addr_list in
         let len = Array.length l
         in if len = 0 then fail Not_found else return l.(Random.int len))
  
let mk_addr ?(port = 6669) addr =
  let __pa_lwt_0 =
    Lwt.catch (fun () -> return @$ (Unix.inet_addr_of_string addr))
      (function | Failure _ -> dns_resolve addr | exn -> Lwt.fail exn)
  in Lwt.bind __pa_lwt_0 (fun ip -> return @$ (U.ADDR_INET (ip, port)))
  
let parse_addr s =
  let l = String.length s in
  let i = try String.rindex s ':' with | Not_found -> l in
  let first_colon = try Some (String.index s ':') with | Not_found -> None in
  let (start_port, addr) =
    if first_colon = (Some i)
    then (* IPv4 or domain name - single colon *) (i, (String.sub s 0 i))
    else (* IPv6 -- detect if port number is present *)
      if (l > 0) && ((s.[0] = '[') && (s.[i - 1] == ']'))
      then (* IPv6 with port of the form "[IPv6]:port" *)
        (i, (String.sub s 1 (i - 1)))
      else (* Assume raw IPv6 *) (l, s) in
  let port =
    if start_port = l
    then None
    else
      (try
         let v = int_of_string (String.sub s (i + 1) ((l - i) - 1))
         in if v < 0 then raise Not_found else Some v
       with | Failure "int_of_string" -> raise Not_found)
  in mk_addr ?port addr
  
(* TODO: check raw IP and not domain name in case of PF_INET6 *)
let string_of_addr addr = let open U
  in
    let __pa_lwt_0 = getnameinfo addr []
    in
      Lwt.bind __pa_lwt_0
        (fun ni ->
...
let raw_addr addr = let open U
  in
    match addr with
    | ADDR_INET (inet, port) -> ((Unix.string_of_inet_addr inet), port)
    | _ -> assert false
      ...
      struct
        ...
        let send (_, output, _) v =
          Lwt.async
            (fun () ->
               Lwt_io.atomic
                 (fun chan ->
                    Lwt_io.write_chars chan (Chan.stream_of_output v))
                 output)
          
        let close (inp, outp, l) =
          Lwt.async
            (fun () ->
               Lwt.bind (Lwt_io.close outp) (fun _ -> Lwt_io.close inp))
          
        let recv (((input, _, _) as t)) =
          Lwt.catch
            (fun () -> Chan.input_of_stream (Lwt_io.read_chars input))
            (function
        ...
        let open_connection addr =
          (Lwt_io.open_connection addr) >>=
            (fun (inp, outp) -> return (inp, outp, (ref [])))
                         ...
                         (Lwt_stream.fold_s
                           ...
                           (fun () ->
                              (catch
                                 (fun () ->
                                    Lwt.bind
                                      (Lwt.bind (Lwt_io.close output)
                                         (fun _ -> Lwt_io.close input))
                                      (fun _ -> return id))
                                 (function
...
module UDP =
  struct
    ...
    let create ?(addr = U.ADDR_INET (Unix.inet_addr_any, 0)) () =
      ...
      let sock = U.socket domain Unix.SOCK_DGRAM 0
      in (U.bind sock addr; sock)
          ...
          (Lwt.async
             (wrap_eintr
                (fun () ->
                   ...
                   | U.Opened ->
                       Lwt.bind (U.sendto fdescr str 0 len [] addr)
                         (fun _ -> return ())
                   | U.Aborted _ -> U.close fdescr
                   | (* TODO ? *) U.Closed -> return ()));
           true)
     ...
     * multiple instances of [recvfrom] running in parallel *)
      let buffer = String.make 65536 ' '
      in
        (wrap_eintr
           (fun () ->
              match U.state fdescr with
              | U.Opened ->
                  (U.recvfrom fdescr buffer 0 65535 []) >|= (fun x -> Some x)
              | U.Aborted _ ->
                  Lwt.bind (U.close fdescr) (fun _ -> return None)
              | (* TODO ? *) U.Closed -> return None)
           ())
          >|=
          (function
           | None -> None
