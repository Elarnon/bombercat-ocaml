...
type client_command = | MOVE of pos * dir | BOMB of pos

type client =
  | COMMAND of string * int * int * client_command | SYNC of string * int

type action = | CLIENT of client_command | NOP of int | DEAD

type stats = { winner : string option }

type server =
  | TURN of int * (action list) Smap.t | (* Imap *) GAMEOVER of int * stats
...
let encode_client =
  ...
  | COMMAND (id, seq, rej, cmd) ->
      D
        (map_of_list
           [ ("Id", (S id)); ("Sequence", (I seq)); ("Reject", (I rej));
...
let decode_client =
  ...
  | D tbl ->
      (try
         ...
         | [ S id; cmd ] ->
             (match gets [ "Sequence"; "Reject" ] tbl with
              | [ I sq; I rj ] ->
                  COMMAND (id, sq, rj, (decode_client_command cmd))
              | _ -> raise (Error "bad"))
         | _ -> raise (Error "worse")
...
let decode_clients =
  function | L lst -> List.map decode_client lst | b -> [ decode_client b ]
...
let encode_stats { winner = winner } =
  match winner with
        ...
        Smap.mapi
          (fun id v ->
             let l =
               if nops = (Some id)
               then v
               else List.filter (function | NOP _ -> false | _ -> true) v
             in L (List.map encode_action l))
          tbl
      in L [ S "TURN"; I i; D btbl ]
  | GAMEOVER (i, stats) -> L [ S "GAMEOVER"; I i; encode_stats stats ]
  
let encode_servers ?nops lst = L (List.map (encode_server ?nops) lst)
...
let decode_servers =
  function
  | L lst ->
      (try [ decode_server @$ (L lst) ]
       with | Error _ -> List.map decode_server lst)
  | b -> [ decode_server b ]
  
(* always fail... *)
let string_to_servers s =
  try
    match Bencode.of_string s with | None -> [] | Some b -> decode_servers b
  with | Bencode.Format_error _ -> [] | Error _ -> []
  
let string_to_clients s =
  try
    match Bencode.of_string s with | None -> [] | Some b -> decode_clients b
  with | Bencode.Format_error _ -> [] | Error _ -> []
