...
let update =
  ...
  in
    fun games -> let open Protocol.Meta
      in
        (List.iter
           (fun
              {
              ...
              let row =
                try
                  let (row, seen) = Hashtbl.find tbl game_id
                  in (seen := true; row)
                with
                | Not_found ->
                    let row = model#append ()
                    in (Hashtbl.add tbl game_id (row, (ref true)); row) in
              let (ip, port) = Network.raw_addr game_addr
              in
                (model#set ~row ~column: col_name game_name;
                 let players =
                   (string_of_int game_nb_players) ^
                     ("/" ^ (string_of_int game_max_players))
                 in
                   (model#set ~row ~column: col_players players;
                    model#set ~row ~column: col_ip
           ...
           (Hashtbl.iter
              (fun id (row, seen) ->
                 ...
                 then seen := false
                   ...
                   (to_remove := id :: !to_remove; ignore (model#remove row)))
              tbl;
...
let create_view ~model ~packing () =
  ...
  in
    (ignore (view#append_column col);
     ...
     let col =
       GTree.view_column ~title: "IP address"
         ~renderer: ((GTree.cell_renderer_text []), [ ("text", col_ip) ]) ()
     in
       (ignore (view#append_column col);
        (* Third column : number of places available *)
        let col =
          GTree.view_column ~title: "Players"
            ~renderer:
              ((GTree.cell_renderer_text []), [ ("text", col_players) ])
            ()
        in (ignore (view#append_column col); view)))
  
let () =
  Lwt_main.run (* Initializes GTK. *) (* Install Lwt<->Glib integration. *)
    (ignore (GMain.init ());
     Lwt_glib.install ();
     (* Thread which is wakeup when the main window is closed. *)
     let (waiter, wakener) = Lwt.wait () in (* Create a window. *)
     let window = GWindow.window () in
     let box = GPack.vbox ~homogeneous: false ~packing: window#add () in
     let __pa_lwt_0 = Network.parse_addr "127.0.0.1:22222"
     in
       Lwt.bind __pa_lwt_0
         (fun addr ->
            let __pa_lwt_0 = Meta.Client.Connection.open_connection addr
            in
              Lwt.bind __pa_lwt_0
                (fun co ->
                   let rec poll () =
