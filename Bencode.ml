...
module StringCharStream = struct
  ...
  let peek (str, pos) =
    return @$ (try Some str.[!pos] with Invalid_argument _ -> None)

  let junk (str, pos) = pos := !pos + 1
  ...
  let is_empty (s, pos) =
    return @$ (!pos >= String.length s)

  let parse (s, pos) f =
    let rem = !pos
    in catch (fun () -> f (s, pos)) (fun e -> (pos := rem; fail e))
...
(* Real stuff *)
type t = | S of string | I of int | L of t list | D of t Smap.t
  ...
  *)
  let nget_rev n s =
    let rec nget_ acc =
      function
      | 0 -> return acc
      | n -> (S.next s) >>= (fun c -> nget_ (c :: acc) (n - 1))
    in nget_ [] n
    ...
    (S.next s) >>=
      (fun c ->
        ...
        then
          (let ioc = int_of_char
	   in
           continue_bint s neg (((value * 10) + (ioc c)) - (ioc '0'))
             endchar)
        else
          if c = endchar
          then return (if neg then - value else value)
          else
            fail
              (Format_error
                 (Format.sprintf
                    ("Unexpected character `%c` while reading a bencoded integer "
                     ^^ "(expecting a digit or `%c`).")
                    c endchar)))
  ...
  let read_bint s endchar =
    (S.peek s) >>=
      (function
      | Some '-' -> (S.junk s) >>= (fun () -> negative_bint s endchar)
      | Some '0' -> (S.junk s) >>= (fun () -> read_bzero s endchar)
      | Some c when ('1' <= c) && (c <= '9') ->
        continue_bint s false 0 endchar
      | Some c ->
        fail
          (Format_error
             (Format.sprintf
  ...
  let rec read_blist s acc =
    (S.peek s) >>=
      (fun c ->
        if c = (Some 'e')
        then (S.junk s) >>= (fun () -> return (List.rev acc))
        else (read_bencode s) >>= (fun v -> read_blist s (v :: acc)))
  and
        ...
        (read_bstring s) >>=
          (fun key -> (* TODO *)
            ...
            then
              fail
                (Format_error
                   "Illegal key order in bencoded dictionary.")
            else
              (read_bencode s) >>=
		(fun v -> read_bdict s (Smap.add key v map) (Some key)))
      | None -> fail S.Empty)
  and (* Reads a string from [s], without prerequisite *) read_bstring 
      s =
    (read_bint s ':') >>=
      (fun len ->
        (nget_rev len s) >>=
          (fun l ->
            let s = String.make len ' '
            in
  ...
  and
    ...
    (S.peek s) >>=
      ...
      | Some 'i' ->
        (S.junk s) >>=
          (fun () -> (read_bint s 'e') >>= (fun i -> return (I i)))
      | Some 'l' ->
        (S.junk s) >>=
          (fun () -> (read_blist s []) >>= (fun l -> return (L l)))
      | Some 'd' ->
        (S.junk s) >>=
          (fun () ->
            (read_bdict s Smap.empty None) >>=
              (fun d -> return @$ (D d)))
      | Some c when ('0' <= c) && (c <= '9') ->
        (read_bstring s) >>= (fun s -> return (S s))
      | Some '\n' -> (S.junk s) >>= (fun () -> read_bencode s)
      | (* TODO: remove *) Some c ->
        fail
          (Format_error
             (Format.sprintf
  ...
  let rec to_string =
    ...
    | L l -> (* [< "l"; concatMap to_stream l; "e" >] *)
      let contents = String.concat "" (List.map to_string l)
      in "l" ^ (contents ^ "e")
    | D m ->
      let beginning =
        Smap.fold (fun k v s -> s ^ ((to_string (S k)) ^ (to_string v)))
          m "d"
      in beginning ^ "e"

  (* ... in order to make it a stream. *)
  let to_stream v = S.of_string (to_string v)
...
let most_to_string len vs =
  let rec travel str n =
    ...
    | elt :: rest ->
      ...
      let nstr = str ^ bstr
      in
      if (String.length nstr) > (len - 2)
      then (("l" ^ (str ^ "e")), n)
      else travel nstr (n + 1) rest
  in travel "" 0 vs
...
let all_to_strings len lst =
  let rec trec acc =
    ...
    | [] -> List.rev acc
    | lst ->
      let (packet, nb) = most_to_string len lst
      in trec (packet :: acc) (discard nb lst)
  in trec [] lst
