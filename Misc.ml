                   ...
                   (let ended_ =
                      List.map
                        (function
                         ...
                         | (Some x, s) -> (x, s))
                        ended''
                    in (ended := ended_; running := running'; step ())))
...
let rec discard n =
  function | _ :: t when n > 0 -> discard (n - 1) t | l -> l
...
let hashtbl_take t k = let v = Hashtbl.find t k in (Hashtbl.remove t k; v)
  
let queue_junk queue = let _ = Queue.take queue in ()
  
let dump_stack stack =
  let lst = ref []
  in
    (while not (Stack.is_empty stack) do lst := (Stack.pop stack) :: !lst
       done;
     !lst)
  
let list_iteri f l =
  let rec iteri n = function | [] -> () | h :: t -> (f n h; iteri (n + 1) t)
  in iteri 0 l
